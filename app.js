            ...
            let badgeListMarkup = roll.badges.map(b => `
                <div class="flex justify-between items-center text-[11px] border-b border-white/5 pb-1 w-full">
                    <span class="text-gray-300 font-mono flex items-center gap-1.5">
                        <span class="text-sm">${b.emoji}</span> <span class="truncate max-w-[180px] font-bold text-gray-200">${b.name}</span>
                    </span>
...
document.getElementById('roll-btn').addEventListener('click', () => {
    ...
    [metaRow, scoreWrapper, feedWrapper].forEach(el => { el.classList.remove('opacity-100', 'translate-y-0'); el.classList.add('opacity-0', 'translate-y-2'); });
    stackOutput.innerHTML = ''; capsuleGlow.style.opacity = '0';
    ...
    badgesEarned.sort((a, b) => { if (tierWeights[a.tier] !== tierWeights[b.tier]) return tierWeights[a.tier] - tierWeights[b.tier]; return a.calculatedEP - b.calculatedEP; });
    ...
    let frameTicks = 0; const maxFrames = 66; let previouslyLockedBoundary = 0;
    const cinematicInterval = setInterval(() => {
        ...
        for (let i = 0; i < 6; i++) {
            ...
            if (i < lockBoundary) {
                if (lockBoundary > previouslyLockedBoundary && i === lockBoundary - 1) digitSpan.className = 'digit-lock-bounce inline-block';
                else digitSpan.className = 'inline-block';
            } else digitSpan.className = 'spinning-digit-dimmed inline-block';
            display.appendChild(digitSpan);
        }
        previouslyLockedBoundary = lockBoundary; synth.tick(); frameTicks++;
        if (frameTicks >= maxFrames) { clearInterval(cinematicInterval); handleTruncationPhase(); }
    }, 60);
    ...
    function handleTruncationPhase() {
        ...
        if (diff > 0) {
            ...
            for (let i = 0; i < diff; i++) {
                setTimeout(() => { spans[i].classList.add('digit-vaporize'); synth.vaporize(); }, delay);
                delay += 250; 
            }
            setTimeout(() => processSystemReveal(), delay + 300);
        } else processSystemReveal();
    }

    function processSystemReveal() {
        display.innerHTML = ''; 
        for (let i = 0; i < naturalStr.length; i++) {
            const span = document.createElement('span'); span.className = 'inline-block'; span.innerText = naturalStr[i]; display.appendChild(span);
        }
        synth.chime(cardRank.name); 

        let tagColorClass = "text-gray-400 border-gray-600 bg-gray-800"; let outerShadow = "0 0 40px rgba(255,255,255,0.05)"; let borderHex = "rgba(255,255,255,0.2)";
        ...
        capsuleGlow.style.background = borderHex; capsuleGlow.style.opacity = '0.5';
        rankTag.innerText = cardRank.name; rankTag.className = `px-3 py-1 rounded-full border backdrop-blur-sm ${tagColorClass}`; percentTag.innerText = percentString;

        [metaRow, scoreWrapper].forEach(el => { el.classList.remove('opacity-0', 'translate-y-2'); el.classList.add('opacity-100', 'translate-y-0'); });

        let countingPoints = 0; const incrementalStep = Math.max(1, Math.ceil(totalEP / 30)); 
        const countingTimer = setInterval(() => {
            countingPoints += incrementalStep;
            if (countingPoints >= totalEP) {
                countingPoints = totalEP; clearInterval(countingTimer);
                sessionLifetimeEP += totalEP; localStorage.setItem('rngdle_ep', sessionLifetimeEP.toString());
                ...
                topRolls.sort((a,b) => b.ep - a.ep); topRolls = topRolls.slice(0, 5); 
                localStorage.setItem('rngdle_topRolls', JSON.stringify(topRolls));
                updateLeaderboard(); loadSequentialBadgeFeed();
            }
    ...
    function loadSequentialBadgeFeed() {
        ...
        function printRowItem() {
            if (cardCursorIndex >= badgesEarned.length) { rollBtn.disabled = false; rollBtn.innerHTML = '<span class="text-xl">🎰</span> Generate Roll'; shareBtn.classList.remove('hidden'); isRolling = false; return; }
            const badge = badgesEarned[cardCursorIndex]; const cardNode = document.createElement('div');
            cardNode.className = "polished-card revealing-card flex flex-col space-y-4 p-5 md:p-6 w-full text-left";
            // ... (keep rest of your badge rendering logic from before)
            // Note: Paste your remaining loadSequentialBadgeFeed logic here
            cardCursorIndex++; setTimeout(printRowItem, 50);
        }
        printRowItem();
    }
});
