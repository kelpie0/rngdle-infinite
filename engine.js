...
// Strobogrammatic Checker (Looks same upside down)
...
// Global evaluation loop for your database items
function evaluateRoll(n) {
    const s = n.toString().padStart(6, '0');
    const digits = s.split('').map(Number);
    let earned = [];

    // Helper functions for sequence tracking
    const countOccurrences = (char) => (s.match(new RegExp(char, 'g')) || []).length;
    
    // Check contiguous blocks
    let maxContig = 1, currentContig = 1, contigChar = s[0];
    let contigBlocks = [];
    ...
    BADGES_DATABASE.forEach(badge => {
        ...
        else if (name === "Single Digit" && new Set(digits).size === 1) match = true;
        else if (name === "Two Digits" && new Set(digits).size === 2) match = true;
        else if (name === "Three Digits" && new Set(digits).size === 3) match = true;
        else if (name === "Four Digits" && new Set(digits).size === 4) match = true;
        else if (name === "Five Digits" && new Set(digits).size === 5) match = true;
        else if (name === "Six Digits" && new Set(digits).size === 6) match = true;
        ...
        // Structural Spacings & Layouts
        else if (name === "Gap One" && Math.abs(digits[0] - digits[5]) === 1) match = true;
        else if (name === "Equilibrium" && digits[0] === digits[5]) match = true;
        else if (name === "Liftoff" && digits[0] > digits[5]) match = true;
        else if (name === "Grounded" && digits[0] < digits[5]) match = true;
    ...
    // Fallback security rule: Always earn base metadata properties if nothing hits
    if (earned.length === 0) {
