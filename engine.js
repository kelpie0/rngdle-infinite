function evaluateRoll(s) {
    ...
    BADGES_DATABASE.forEach(badge => {
        ...
        if (name.startsWith("Exact ")) {
        ...
        else if (name === "6 Consecutive Numbers") {
            for(let i=0; i<=s.length-6; i++) {
                let sub = digits.slice(i, i+6);
                let goesUp = sub[1] === sub[0]+1 && sub[2] === sub[1]+1 && sub[3] === sub[2]+1 && sub[4] === sub[3]+1 && sub[5] === sub[4]+1;
                let goesDown = sub[1] === sub[0]-1 && sub[2] === sub[1]-1 && sub[3] === sub[2]-1 && sub[4] === sub[3]-1 && sub[5] === sub[4]-1;
                if (goesUp || goesDown) { match = true; break; }
            }
        }
        else if (name === "5 Consecutive Numbers") {
            for(let i=0; i<=s.length-5; i++) {
                let sub = digits.slice(i, i+5);
                let goesUp = sub[1] === sub[0]+1 && sub[2] === sub[1]+1 && sub[3] === sub[2]+1 && sub[4] === sub[3]+1;
                let goesDown = sub[1] === sub[0]-1 && sub[2] === sub[1]-1 && sub[3] === sub[2]-1 && sub[4] === sub[3]-1;
                if (goesUp || goesDown) { match = true; break; }
            }
        }
            ...
            for(let i=0; i<=s.length-4; i++) {
                ...
                let goesUp = sub[1] === sub[0]+1 && sub[2] === sub[1]+1 && sub[3] === sub[2]+1;
                let goesDown = sub[1] === sub[0]-1 && sub[2] === sub[1]-1 && sub[3] === sub[2]-1;
                if (goesUp || goesDown) { match = true; break; }
            }
        }
            ...
            for(let i=0; i<=s.length-3; i++) {
                ...
                let goesUp = sub[1] === sub[0]+1 && sub[2] === sub[1]+1;
                let goesDown = sub[1] === sub[0]-1 && sub[2] === sub[1]-1;
                if (goesUp || goesDown) { match = true; break; }
            }
        }
